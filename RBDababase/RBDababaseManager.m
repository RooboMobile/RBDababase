...
- (void)createTable:(Class)modelClass complete:(void(^)(BOOL flag))complete{
    ...
    [self createTable:modelClass autoCloseDB:NO complete:complete];
}

- (void)insertModel:(id)model complete:(void(^)(BOOL flag))complete{
    [self insertModel:model autoCloseDB:YES complete:complete];
}

- (void)insertModel:(id)model autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(BOOL flag))complete{
    if ([model isKindOfClass:[NSArray class]] || [model isKindOfClass:[NSMutableArray class]]) {
        NSArray *modelArr = (NSArray *)model;
        [self insertModelArr:modelArr autoCloseDB:autoCloseDB complete:complete];
    }
    else{
        [self insertSingleModel:model inAsync:YES autoCloseDB:autoCloseDB complete:complete];
    }
}

- (void)searchModel:(Class)modelClass byID:(NSString *)FLDBID complete:(void(^)(id model))complete{
    [self searchModel:modelClass byID:FLDBID autoCloseDB:YES complete:complete];
}

- (void)searchModelArr:(Class)modelClass complete:(void(^)(NSArray *modelArr))complete{
    [self searchModelArr:modelClass autoCloseDB:YES complete:complete];
}
...
- (void)modifyModel:(id)model byID:(NSString *)FLDBID complete:(void(^)(BOOL flag))complete{
    [self modifyModel:model byID:FLDBID autoCloseDB:YES complete:complete];
}
...
- (void)dropTable:(Class)modelClass complete:(void(^)(BOOL flag))complete{
    [self dropTable:modelClass autoCloseDB:YES complete:complete];
}
...
- (void)deleteAllModel:(Class)modelClass complete:(void(^)(BOOL flag))complete{
    [self deleteAllModel:modelClass autoCloseDB:YES complete:complete];
}

- (void)deleteAllModel:(Class)modelClass autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(BOOL flag))complete{
    __block BOOL success = true;
    __weak typeof(self) weakSelf = self;
...
- (void)deleteModel:(Class)modelClass byId:(NSString *)FLDBID complete:(void(^)( BOOL flag))complete{
    [self deleteModel:modelClass byId:FLDBID autoCloseDB:YES complete:complete];
}
 ...
 *  指定的表是否存在
 */
- (void)isExitTable:(Class)modelClass autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(BOOL flag))complete{
    __block BOOL success = true;
    __weak typeof(self) weakSelf = self;
    //把任务包装到事务里
    FLDISPATCH_ASYNC_GLOBAL(^{
 ...
 *  创表
 */
- (void)createTable:(Class)modelClass autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(BOOL flag))complete{
...
- (BOOL)create:(FMDatabase *)db table:(Class)modelClass autoCloseDB:(BOOL)autoCloseDB{
    if ([db open]) {
        // 创表,判断是否已经存在
        if ([self isExit:db table:modelClass autoCloseDB:NO]) {
            if (autoCloseDB) {
                [self closeDB:db];
            }
            return YES;
 ...
 *  插入数据
 */
- (void)insertSingleModel:(id)model inAsync:(BOOL)inAsync autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(BOOL flag))complete{
    __block BOOL success = true;
    __weak typeof(self) weakSelf = self;
    if (inAsync) {
        FLDISPATCH_ASYNC_GLOBAL(^{
...
- (void)insertModelArr:(NSArray *)modelArr autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(BOOL flag))complete{
    ...
    FLDISPATCH_ASYNC_GLOBAL(^{
        ...
        for (NSInteger index = 0; index < modelArr.count; index ++) {
            ...
            [strongSelf insertSingleModel:model inAsync:NO autoCloseDB:autoCloseDB complete:nil];
        }
    });
}


- (BOOL)insert:(FMDatabase *)db modelArr:(NSArray *)modelArr autoCloseDB:(BOOL)autoCloseDB{
    ...
    for (NSInteger index = 0; index < modelArr.count; index ++) {
        id model = modelArr[index];
        // 处理过程中不关闭数据库
        success = [self insert:db model:model autoCloseDB:NO];
    }
    // 操作完关闭数据库
    if (autoCloseDB) {
        [self closeDB:db];
...
- (BOOL)insert:(FMDatabase *)db model:(id)model autoCloseDB:(BOOL)autoCloseDB{
    ...
    if ([db open]) {
        ...
        if (![self isExit:db table:[model class] autoCloseDB:NO]) {
            // 第二步处理完不关闭数据库
            BOOL success = [self create:db table:[model class] autoCloseDB:NO];
            success = [db open];
            //            if (success) {
            ...
            if (success) {
                ...
                id judgeModle = [self search:db model:[model class] byID:dbid autoCloseDB:YES];
                
                if ([[judgeModle valueForKey:@"FLDBID"] isEqualToString:dbid]) {
                    BOOL updataSuccess = [self modify:db model:model byID:dbid autoCloseDB:NO];
                    if (autoCloseDB) {
                        [self closeDB:db];
                    }
                    ...
                    insertSuccess = [self create:db table:[model class] autoCloseDB:NO];
 ...
 *  查询
 */
- (void)searchModel:(Class)modelClass byID:(NSString *)FLDBID autoCloseDB:(BOOL)autoCloseDB complete:(void(^)(id model))complete{
    __block BOOL success = true;
    __weak typeof(self) weakSelf = self;
    //把任务包装到事务里
    FLDISPATCH_ASYNC_GLOBAL(^{
...
- (id)search:(FMDatabase *)db model:(Class)modelClass byID:(NSString *)FLDBID autoCloseDB:(BOOL)autoCloseDB{
    
    if ([db open]) {
        BOOL success = [self isExit:db table:modelClass autoCloseDB:NO];
        if (!success) {
            if (autoCloseDB) {
                [self closeDB:db];
            }
...
- (NSArray *)search:(FMDatabase *)db modelArr:(Class)modelClass autoCloseDB:(BOOL)autoCloseDB{
    if ([db open]) {
        BOOL success = [self isExit:db table:modelClass autoCloseDB:NO];
        if (!success) {
            if (autoCloseDB) {
...
- (BOOL)modify:(FMDatabase *)db model:(id)model byID:(NSString *)FLDBID autoCloseDB:(BOOL)autoCloseDB{
    if ([db open]) {
        // 判断是否已经存在
        BOOL success = [self isExit:db table:[model class] autoCloseDB:NO];
        if (!success) {
            if (autoCloseDB) {
                [self closeDB:db];
            }
 ...
 *  删除
 */
- (BOOL)drop:(FMDatabase *)db table:(Class)modelClass autoCloseDB:(BOOL)autoCloseDB{
    if ([db open]) {
        // 此时需要判断完关闭数据库，再重新打开
        BOOL success = [self isExit:db table:modelClass autoCloseDB:YES];
        if (!success) {
            if (autoCloseDB) {
                [self closeDB:db];
            }
...
- (BOOL)delete:(FMDatabase *)db model:(Class)modelClass byId:(NSString *)FLDBID autoCloseDB:(BOOL)autoCloseDB {
    if ([db open]) {
        // 删除数据
        BOOL success = [self isExit:db table:modelClass autoCloseDB:NO];
        if (!success) {
        ...
        // 判断是否存在
        if ([self search:db model:modelClass byID:FLDBID autoCloseDB:NO]) {
...
- (BOOL)delete:(FMDatabase *)db allModel:(Class)modelClass autoCloseDB:(BOOL)autoCloseDB{
    if ([db open]) {
        // 删除数据
        BOOL success = [self isExit:db table:modelClass autoCloseDB:NO];
        if (!success) {
        ...
        // 数据是否存在
        NSArray *modelArr = [self search:db modelArr:modelClass autoCloseDB:NO];
